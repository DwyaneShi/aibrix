    ...
    async def allocate(
        ...
        for key_batch in self._cache_block_key_batches(prefix, query):
            ...
            for i in range(len(tasks)):
                ...
                def on_seal(x: torch.Tensor, y: int, z: int) -> None:
                    self._enqueue_mr_op(
                        connector_mr, lambda mr: self._backend.seal(mr)
                    )
    ...
    async def acquire(
        ...
        for key_batch in self._cache_block_key_batches(prefix, query):
            ...
            for i in range(len(tasks)):
                ...
                def on_release(x: torch.Tensor, y: int, z: int) -> None:
                    self._enqueue_mr_op(
                        connector_mr, lambda mr: self._backend.release(mr)
                    )
